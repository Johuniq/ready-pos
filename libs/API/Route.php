...
namespace Readypos\Libs\API;

use Readypos\Libs\API\ApiRouteException;
...
class Route {
	...
	/**
	 * Prepare a lazy callback that defers class instantiation.
	 *
	 * Instead of instantiating the controller class during route registration,
	 * this returns a closure that instantiates the class only when the route
	 * is actually called. This prevents one broken controller from blocking
	 * all other routes from being registered.
	 *
	 * @since 1.0.0
	 *
	 * @param mixed $callback callback.
	 ...
	 * @return mixed
	 */
	private static function prepare_lazy_callback( $callback ) {
		if ( is_string( $callback ) && self::str_has( $callback, '@' ) ) {
			$arr    = explode( '@', $callback, 2 );
			$class  = $arr[0];
			$method = $arr[1];
